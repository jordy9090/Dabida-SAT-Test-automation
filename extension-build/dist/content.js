...
(() => {
      ...
      (function(global, factory) {
        ...
        var codePointsToCharacterClasses = function(codePoints, lineBreak2) {
          ...
          codePoints.forEach(function(codePoint, index) {
            ...
            indices.push(index);
            if (classType === CJ) {
              return types.push(lineBreak2 === "strict" ? NS : ID2);
            }
            if (classType === SA) {
              return types.push(AL);
            }
        ...
        var _lineBreakAtIndex = function(codePoints, classTypes, indicies, index, forbiddenBreaks) {
          ...
          if (current === PR && [ID2, EB, EM].indexOf(next) !== -1 || [ID2, EB, EM].indexOf(current) !== -1 && next === PO) {
            return BREAK_NOT_ALLOWED$1;
          }
        ...
        var cssFormattedClasses = function(codePoints, options) {
          ...
          if (options.wordBreak === "break-all" || options.wordBreak === "break-word") {
            classTypes = classTypes.map(function(type) {
              return [NU, AL, SA].indexOf(type) !== -1 ? ID2 : type;
            });
          }
  ...
  async function waitFor(predicate, { timeout = 2e4, description = "condition", root = document } = {}) {
    ...
    const visit = (win) => {
      for (let i = 0; i < win.frames.length; i += 1) {
        try {
          ...
          candidates.push(frame.document);
          visit(frame);
        } catch {
        }
      }
  ...
  async function clickAndWait(element, changed, description, root = document) {
    if (!element || !visible(element)) throw new Error(`Cannot click ${description}`);
    element.scrollIntoView?.({ block: "center" });
    element.click();
    return waitFor(changed, { timeout: 2e4, description: `${description} state transition`, root });
  }
  ...
  function signature(problem) {
    let hash = 2166136261;
    for (const char of norm(problem.question)) hash = Math.imul(hash ^ char.charCodeAt(0), 16777619);
    return `${problem.section}:${problem.module}:${problem.problemNumber}:${(hash >>> 0).toString(16)}`;
  }
  function dedupeAndValidate(problems) {
    const map = /* @__PURE__ */ new Map();
    for (const p of problems) map.set(signature(validateProblem(p)), p);
    return [...map.values()];
  }
    ...
    async run() {
      ...
      while (guard++ < 250) {
        ...
        if (state.phase === "TRANSITION") {
          await this.transition(root);
          continue;
        }
        if (state.phase !== "QUESTION") {
          await waitFor(() => sectionState(satDocument()).phase !== "UNKNOWN", { timeout: 3e4, description: "SAT UI", root });
          continue;
        }
  ...
  async function startGeminiPrompt() {
  ...
  function getJsPDF() {
    ...
    if (!ctor) throw new Error("jsPDF is unavailable");
    return ctor;
  }
  function addWrapped(doc, text, x, y, width, line = 6) {
    for (const part of doc.splitTextToSize(String(text || ""), width)) {
      if (y > 275) {
        doc.addPage();
        y = 20;
      }
  ...
  function makeDoc(section, problems, answers) {
    const Doc = getJsPDF(), doc = new Doc({ unit: "mm", format: "a4" });
    let y = 20;
    doc.setFontSize(16);
    doc.text(`SAT ${section === "reading" ? "Reading" : "Math"} ${answers ? "Answers & Explanations" : "Problems"}`, 20, y);
    y += 12;
    for (const p of problems) {
      ...
      doc.text(`Module ${p.module} - Problem ${p.problemNumber}`, 20, y);
      y += 7;
      doc.setFont(void 0, "normal");
      doc.setFontSize(10);
      if (answers) {
        y = addWrapped(doc, `Answer: ${p.correctAnswer}`, 20, y, 170);
        y = addWrapped(doc, `Explanation: ${p.explanation}`, 20, y, 170);
      } else {
        ...
        for (const figure of p.figures || []) {
          try {
            const h = Math.min(70, 160 * figure.height / figure.width);
            if (y + h > 275) {
              doc.addPage();
              y = 20;
            }
            doc.addImage(figure.dataUrl, "PNG", 25, y, 160, h);
            y += h + 5;
          } catch {
          }
        }
  ...
  function buildPdfDocuments(input) {
  ...
  async function downloadFour(documents) {
    const date = (/* @__PURE__ */ new Date()).toISOString().slice(0, 10), files = [["readingProblems", `SAT_Reading_Problems_${date}.pdf`], ["readingAnswers", `SAT_Reading_Answers_${date}.pdf`], ["mathProblems", `SAT_Math_Problems_${date}.pdf`], ["mathAnswers", `SAT_Math_Answers_${date}.pdf`]];
    for (const [key, name] of files) {
      documents[key].save(name);
      await new Promise((resolve) => setTimeout(resolve, 300));
    }
  }

  // src/runtime/app.js
  var ID = "gemini-sat-pdf-export-btn";
  function setStatus(button, text, error = false) {
    button.textContent = text;
    button.dataset.state = error ? "Error" : text;
    button.style.background = error ? "#b3261e" : "#1a73e8";
  }
  ...
  function boot() {
    ...
    const install = () => {
      ...
      button.id = ID;
      setStatus(button, "Export to PDF");
      button.addEventListener("click", async () => {
        ...
        try {
          ...
          const runner = new SATRunner((text) => setStatus(button, text), {
            maxProblems: smoke ? 1 : void 0,
            onCollect(problem, problems2) {
              button.dataset.collectedCount = String(problems2.length);
          ...
          setStatus(button, "Building PDFs");
          await downloadFour(buildPdfDocuments(problems));
          setStatus(button, "Done");
        } catch (error) {
          console.error("[Gemini SAT Exporter]", error);
          setStatus(button, `Error: ${error.message}`, true);
        } finally {
          button.dataset.running = "false";
          button.disabled = false;
        }
      });
      button.dataset.buildSha = BUILD_INFO.gitSha;
      button.dataset.buildTimestamp = BUILD_INFO.timestamp;
