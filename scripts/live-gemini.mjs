...
let gitSha = 'nogit';
try {
  gitSha = execFileSync('git', ['rev-parse','--short','HEAD'], { cwd: root, encoding:'utf8' }).trim() || 'nogit';
} catch (error) {
  const reason = error?.code === 'ENOENT' ? 'git executable not found' : 'git metadata unavailable';
  console.warn(`Git metadata optional: ${reason}; continuing with gitSha=nogit`);
}
const logs = [], errors = [], downloads = [];
let page, context, playwrightBrowser, puppeteerBrowser;

console.log(`Mode: ${mode}`);
console.log(`Extension: ${extensionPath}`);
console.log(`Git SHA: ${gitSha}`);
console.log(`Build timestamp: ${build.timestamp}`);
console.log(`Chrome executable: ${chromePath} (installed Google Chrome)`);
...
async function evidence(error) {
  ...
  if (html) writeFileSync(path.join(resultPath,'page.html'), html);
...
try {
  ...
  playwrightBrowser = await chromium.connectOverCDP(puppeteerBrowser.wsEndpoint());
  context = playwrightBrowser.contexts()[0];
  console.log('Installed unpacked extension through Puppeteer Chrome extension support.');
